...
RpcServer::RpcServer(std::string service_name,
    ...
    : name_(std::move(service_name)),
      ...
      io_context_(nullptr), logger_("coro_rpcserver") {
...
absl::Status RpcServer::Run(boost::asio::io_context *ioc) {
  ...
  boost::asio::co_spawn(
      *io_context_,
      [server = shared_from_this()]() -> boost::asio::awaitable<void> {
        co_await ListenerCoroutine(server);
      },
      boost::asio::detached);
...
RpcServer::ListenerCoroutine(std::shared_ptr<RpcServer> server) {
  while (server->running_) {
    ...
    auto fd = co_await async_wait_either(sub_fd, interrupt_fd);
    if (!fd.ok()) {
...
boost::asio::awaitable<absl::Status>
RpcServer::PublishRpcServerResponse(
  ...
  for (;;) {
    ...
    if (*buffer == nullptr) {
      ...
      auto status = co_await async_wait_either(pub_fd, interrupt_fd);
      if (!status.ok()) {
...
RpcServer::HandleOpen(uint64_t client_id,
  ...
  auto s = co_await CreateSession(client_id);
  if (!s.ok()) {
...
boost::asio::awaitable<absl::StatusOr<std::shared_ptr<Session>>>
      ...
      boost::asio::co_spawn(
          *io_context_,
          [server = shared_from_this(), session,
           method_instance]() -> boost::asio::awaitable<void> {
...
boost::asio::awaitable<void> RpcServer::SessionRequestCoroutine(
  ...
  while (server->running_) {
    ...
    auto s = co_await async_wait_either(sub_fd, interrupt_fd);
    if (!s.ok()) {
      server->logger_.Log(toolbelt::LogLevel::kError,
                          "Error waiting for request: %s",
      ...
      auto reply_fn =
          [server, queue, method_name = method_instance->method->name,
           ...
           client_id = request.client_id()](
            ...
            if (absl::Status st = queue->pipe.Write(std::move(item));
                !st.ok()) {
              server->logger_.Log(toolbelt::LogLevel::kError,
...
boost::asio::awaitable<void> RpcServer::SessionResponseCoroutine(
  ...
  int interrupt_fd = server->interrupt_pipe_.ReadFd().Fd();
  while (server->running_) {
    auto w = co_await async_wait_either(queue.pipe.ReadFd(), interrupt_fd);
    if (!w.ok()) {
      server->logger_.Log(toolbelt::LogLevel::kError,
                          "Error waiting for reply queue: %s",
    ...
    auto item_or = co_await queue.pipe.Read();
    if (!item_or.ok()) {
      server->logger_.Log(toolbelt::LogLevel::kError,
...
boost::asio::awaitable<void> RpcServer::SessionStreamingMethodCoroutine(
    std::shared_ptr<RpcServer> server, std::shared_ptr<Session> session,
    std::shared_ptr<MethodInstance> method_instance) {
  while (server->running_) {
    int sub_fd = method_instance->request_subscriber->GetPollFd().fd;
...
boost::asio::awaitable<void> RpcServer::SendRpcError(
    std::shared_ptr<RpcServer> server, std::shared_ptr<Session> session,
...
boost::asio::awaitable<void> RpcServer::SendStreamRpcResponse(
    std::shared_ptr<RpcServer> server, std::shared_ptr<Session> session,
    std::shared_ptr<MethodInstance> method_instance, const RpcRequest &request,
