...
ClientImpl::CreatePublisher(const std::string &channel_name, int slot_size,
  ...
  if (!opts.IsReliable()) {
    ...
    MessageSlot *slot = channel->FindFreeSlot(
        false, channel->GetPublisherId(),
        [ this, channel = channel.get() ](ChannelLock * lock) {
          absl::StatusOr<bool> ok = ReloadBuffersIfNecessary(channel, lock);
          if (!ok.ok()) {
            return false;
          }
          return *ok;
  ...
  if (publisher->IsReliable() && publisher->CurrentSlot() == nullptr) {
    ...
    MessageSlot *slot =
        publisher->FindFreeSlot(true, publisher->GetPublisherId(),
                                [this, publisher](ChannelLock *lock) {
                                  absl::StatusOr<bool> ok =
                                      ReloadBuffersIfNecessary(publisher, lock);
                                  if (!ok.ok()) {
                                    return false;
                                  }
                                  return *ok;
...
absl::StatusOr<const Message> ClientImpl::PublishMessage(PublisherImpl *publisher,
                                                     int64_t message_size) {
  return PublishMessageInternal(publisher, message_size, /*omit_prefix=*/false);
}

absl::StatusOr<const Message>
ClientImpl::PublishMessageInternal(PublisherImpl *publisher, int64_t message_size,
                               bool omit_prefix) {
  ...
  Channel::PublishedMessage msg = publisher->ActivateSlotAndGetAnother(
      ...
      [this, publisher](ChannelLock *lock) {
        absl::StatusOr<bool> ok = ReloadBuffersIfNecessary(publisher, lock);
        if (!ok.ok()) {
          return false;
        }
        return *ok;
...
ClientImpl::ReadMessageInternal(SubscriberImpl *subscriber, ReadMode mode,
  ...
  case ReadMode::kReadNext:
    new_slot = subscriber->NextSlot([this, subscriber](ChannelLock *lock) {
      absl::StatusOr<bool> ok = ReloadBuffersIfNecessary(subscriber, lock);
      if (!ok.ok()) {
        return false;
      }
      return *ok;
  ...
  case ReadMode::kReadNewest:
    new_slot = subscriber->LastSlot([this, subscriber](ChannelLock *lock) {
      absl::StatusOr<bool> ok = ReloadBuffersIfNecessary(subscriber, lock);
      if (!ok.ok()) {
        return false;
      }
...
absl::StatusOr<const Message> ClientImpl::ReadMessage(SubscriberImpl *subscriber,
                                                  ReadMode mode) {
...
absl::StatusOr<const Message>
ClientImpl::FindMessageInternal(SubscriberImpl *subscriber, uint64_t timestamp) {
...
absl::StatusOr<const Message> ClientImpl::FindMessage(SubscriberImpl *subscriber,
                                                  uint64_t timestamp) {
...
absl::StatusOr<bool> ClientImpl::ReloadBuffersIfNecessary(ClientChannel *channel,
                                                      ChannelLock *lock) {
  ...
  // Unlock channel while talking to the server.
  lock->Unlock();
...
absl::Status ClientImpl::ReloadSubscribersIfNecessary(PublisherImpl *publisher) {
  if (absl::Status status = CheckConnected(); !status.ok()) {
    return status;
  }
...
absl::Status
ClientImpl::SendRequestReceiveResponse(const Request &req, Response &response,
                                   std::vector<toolbelt::FileDescriptor> &fds) {
