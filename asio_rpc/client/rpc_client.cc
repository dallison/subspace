...
absl::Status RpcClient::Open(std::chrono::nanoseconds timeout,
  ...
  return OpenService(timeout, yield);
}
...
absl::Status RpcClient::Open(std::chrono::nanoseconds timeout) {
  ...
  boost::asio::spawn(
      ioc,
      [this, timeout, &result](boost::asio::yield_context yield) {
        result = OpenService(timeout, yield);
      },
      boost::asio::detached);
  ioc.run();
  return result;
...
absl::Status RpcClient::Close(std::chrono::nanoseconds timeout,
  ...
  auto status = CloseService(timeout, yield);
  if (!status.ok()) {
...
absl::Status RpcClient::Close(std::chrono::nanoseconds timeout) {
  ...
  boost::asio::spawn(
      ioc,
      [this, timeout, &result](boost::asio::yield_context yield) {
        result = CloseService(timeout, yield);
      },
      boost::asio::detached);
  ioc.run();
  if (!result.ok()) {
...
RpcClient::PublishServerRequest(const subspace::RpcServerRequest &req,
  ...
  for (;;) {
    ...
    auto wait_status =
        async_wait_readable_timeout(pub_fd, timeout, yield);
    if (!wait_status.ok()) {
      return absl::InternalError(absl::StrFormat(
...
RpcClient::ReadServerResponse(int request_id, std::chrono::nanoseconds timeout,
  ...
  for (;;) {
    ...
    auto wait_status =
        async_wait_readable_timeout(sub_fd, timeout, yield);
    if (!wait_status.ok()) {
...
absl::Status RpcClient::OpenService(std::chrono::nanoseconds timeout,
  ...
  auto pub_status = PublishServerRequest(req, timeout, yield);
  if (!pub_status.ok()) {
    return pub_status;
  }

  auto resp = ReadServerResponse(request_id, timeout, yield);
  if (!resp.ok()) {
    return absl::InternalError(absl::StrFormat(
...
RpcClient::InvokeMethod(int method_id, const google::protobuf::Any &request,
  ...
  *req.mutable_argument() = request;
  ...
  for (;;) {
    ...
    auto wait_status = async_wait_readable_timeout(
        pub_fd, timeout, yield);
    if (!wait_status.ok()) {
      return absl::InternalError(absl::StrFormat(
          "Failed to wait for message buffer: %s", wait_status.ToString()));
    }
...
absl::Status RpcClient::InvokeMethod(
    ...
    std::function<void(std::shared_ptr<RpcClient>, uint64_t, int, int,
                       ...
                       const RpcResponse *)>
        response_handler,
    std::chrono::nanoseconds timeout, boost::asio::yield_context yield) {
  if (closed_) {
    return absl::InternalError("Client is closed");
  }
  auto it = methods_.find(method_id);
...
absl::Status
RpcClient::CancelRequest([[maybe_unused]] uint64_t client_id,
                          [[maybe_unused]] int session_id, int request_id,
