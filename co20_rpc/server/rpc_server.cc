...
RpcServer::RpcServer(std::string service_name,
    ...
    : name_(std::move(service_name)),
      ...
      scheduler_(nullptr), logger_("co20_rpcserver") {
...
absl::Status RpcServer::RegisterMethod(
  ...
  return RegisterMethodAsync(
      ...
      [method, callback = std::move(callback)](
        ...
        auto status = co_await callback(req, res.get());
        if (!status.ok()) {
...
absl::Status RpcServer::RegisterMethod(
  ...
  return RegisterMethodAsync(
      ...
      [method, callback = std::move(callback)](
          ...
          std::function<void(std::string)> error_reply)
          -> co20::ValueTask<void> {
        auto status = co_await callback(req);
        if (!status.ok()) {
          error_reply(absl::StrFormat("Error executing method %s: %s", method,
                                      status.ToString()));
          co_return;
...
absl::Status RpcServer::RegisterMethod(
    ...
    std::string_view response_type,
    std::function<co20::ValueTask<absl::Status>(
        const google::protobuf::Any &, AnyStreamWriter &)>
        callback,
    MethodOptions &&options) {
  if (methods_.find(method) != methods_.end()) {
    return absl::AlreadyExistsError("Method already registered: " + method);
...
absl::Status RpcServer::Run(co20::Scheduler *scheduler) {
  ...
  running_ = true;
  scheduler_->Spawn(
      [server = shared_from_this()](co20::Coroutine &co) -> co20::Task {
        co_await ListenerCoroutine(server, co);
        co_return;
      },
      "RPC listener", interrupt_pipe_.ReadFd().Fd());
...
co20::ValueTask<void>
RpcServer::ListenerCoroutine(std::shared_ptr<RpcServer> server,
                             co20::Coroutine &co) {
  while (server->running_) {
    int sub_fd = server->request_receiver_->GetPollFd().fd;
    int result_fd = co_await co.Wait(sub_fd, POLLIN);

    if (result_fd == co.GetInterruptFd()) {
      break;
    }
    if (result_fd != sub_fd) {
      server->logger_.Log(toolbelt::LogLevel::kError,
                          "Unexpected fd from Wait: %d", result_fd);
      continue;
    }
    ...
    for (;;) {
      ...
      if (auto status =
              co_await server->HandleIncomingRpcServerRequest(std::move(*msg));
          !status.ok()) {
        server->logger_.Log(toolbelt::LogLevel::kError,
...
RpcServer::HandleIncomingRpcServerRequest(subspace::Message msg) {
  ...
  if (!request.ParseFromArray(msg.buffer, msg.length)) {
    co_return absl::InvalidArgumentError("Failed to parse RpcRequest");
  }
  ...
  case subspace::RpcServerRequest::kOpen: {
    if (auto status = co_await HandleOpen(request.client_id(), request.open(),
                                          response.mutable_open());
        !status.ok()) {
  ...
  case subspace::RpcServerRequest::kClose: {
    if (auto status = co_await HandleClose(request.client_id(), request.close(),
                                           response.mutable_close());
        !status.ok()) {
      response.set_error(status.ToString());
    }
    break;
  ...
  co_return co_await PublishRpcServerResponse(response);
}

co20::ValueTask<absl::Status> RpcServer::PublishRpcServerResponse(
  ...
  for (;;) {
    ...
    if (!buffer.ok()) {
      co_return buffer.status();
    }
    if (*buffer == nullptr) {
      int pub_fd = response_publisher_->GetPollFd().fd;
      int result_fd = co_await co20::Wait(pub_fd, POLLIN);
      if (result_fd == co20::self->GetInterruptFd()) {
        co_return absl::OkStatus();
      }
      continue;
    }
    ...
    if (!response.SerializeToArray(*buffer, length)) {
      co_return absl::InternalError("Failed to serialize RpcServerResponse");
    }
...
co20::ValueTask<absl::Status>
RpcServer::HandleClose(uint64_t client_id,
  ...
  if (it == sessions_.end()) {
    co_return absl::NotFoundError("Session not found");
  }
    ...
    if (method->IsStreaming()) {
      scheduler_->Spawn(
          [server = shared_from_this(), session,
           method_instance](co20::Coroutine &co) -> co20::Task {
            co_await SessionStreamingMethodCoroutine(std::move(server), session,
                                                     method_instance, co);
            co_return;
          },
          absl::StrFormat("Session %d Method %s", session->session_id,
                          method->name.c_str()),
          interrupt_pipe_.ReadFd().Fd());
    } else {
      ...
      scheduler_->Spawn(
          [server = shared_from_this(), session,
           method_instance](co20::Coroutine &co) -> co20::Task {
            co_await SessionRequestCoroutine(std::move(server), session,
                                             method_instance, co);
            co_return;
          },
      ...
      scheduler_->Spawn(
          [server = shared_from_this(), session,
           method_instance](co20::Coroutine &co) -> co20::Task {
            co_await SessionResponseCoroutine(std::move(server), session,
                                              method_instance, co);
            co_return;
          },
...
co20::ValueTask<void> RpcServer::SessionRequestCoroutine(
    std::shared_ptr<RpcServer> server, std::shared_ptr<Session> session,
    std::shared_ptr<MethodInstance> method_instance, co20::Coroutine &co) {
  while (server->running_) {
    ...
    if (result_fd != sub_fd) {
      server->logger_.Log(toolbelt::LogLevel::kError,
                          "Error waiting for request: unexpected fd %d",
                          result_fd);
      co_return;
    }
    ...
    for (;;) {
      ...
      // Reply handles tied to this request.  The handler runs on this request
      // coroutine, so the completed result is enqueued from here and the
      // response coroutine routes it back to the client that issued the
      // request.  The pipe write is a small, non-blocking, single-syscall
      // transfer that the response coroutine drains, so no locking is needed.
      auto queue = method_instance->reply_queue;
      auto reply_fn =
          [server, queue, method_name = method_instance->method->name,
           session_id = session->session_id, request_id = request.request_id(),
           client_id = request.client_id()](
            ...
            if (absl::Status s = queue->pipe.Write(std::move(item)); !s.ok()) {
              server->logger_.Log(toolbelt::LogLevel::kError,
                                  "Failed to enqueue reply for method %s: %s",
                                  method_name.c_str(), s.ToString().c_str());
            }
          };
      auto error_fn =
          [server, queue, method_name = method_instance->method->name,
           session_id = session->session_id, request_id = request.request_id(),
           client_id = request.client_id()](std::string error_msg) {
            auto item = std::make_shared<internal::ReplyItem>();
            item->session_id = session_id;
            item->request_id = request_id;
            item->client_id = client_id;
      ...
      server->logger_.Log(toolbelt::LogLevel::kDebug, "Calling method %s",
                          method_instance->method->name.c_str());
      co_await method_instance->method->async_callback(
          request.argument(), std::move(reply_fn), std::move(error_fn));
    }
  }
  co_return;
}

co20::ValueTask<void> RpcServer::SessionResponseCoroutine(
    std::shared_ptr<RpcServer> server,
    [[maybe_unused]] std::shared_ptr<Session> session,
    std::shared_ptr<MethodInstance> method_instance, co20::Coroutine &co) {
  auto &queue = *method_instance->reply_queue;
  while (server->running_) {
    // Read suspends on the pipe until a reply arrives (or the interrupt fd
    // fires, reported as a CancelledError).
    std::shared_ptr<internal::ReplyItem> item;
    absl::Status read_status = co_await queue.pipe.Read(item);
    if (!read_status.ok()) {
      if (absl::IsCancelled(read_status)) {
        break;
      }
      server->logger_.Log(toolbelt::LogLevel::kError,
    ...
    for (;;) {
      ...
      int wait_fd = co_await co.Wait(pub_fd, POLLIN);
      if (wait_fd == co.GetInterruptFd()) {
        co_return;
      }
    }
    if (!got_buffer) {
...
co20::ValueTask<void> RpcServer::SessionStreamingMethodCoroutine(
    std::shared_ptr<RpcServer> server, std::shared_ptr<Session> session,
    std::shared_ptr<MethodInstance> method_instance, co20::Coroutine &co) {
  while (server->running_) {
    int sub_fd = method_instance->request_subscriber->GetPollFd().fd;
    ...
    if (cancel_interrupt != nullptr) {
      server->scheduler_->Spawn(
          [server, session, method_instance, writer,
           cancel_interrupt](co20::Coroutine &cancel_co) -> co20::Task {
    ...
    if (!method_status.ok()) {
      ...
      co_await SendRpcError(
          server, session, method_instance, request,
...
co20::ValueTask<void> RpcServer::SendRpcError(
    std::shared_ptr<RpcServer> server, std::shared_ptr<Session> session,
  ...
  for (;;) {
    ...
    int pub_fd = method_instance->response_publisher->GetPollFd().fd;
    int wait_fd = co_await co20::Wait(pub_fd, POLLIN);
    if (wait_fd == co20::self->GetInterruptFd()) {
      co_return;
    }
  }
  if (!response.SerializeToArray(*buffer, length)) {
...
co20::ValueTask<void> RpcServer::SendStreamRpcResponse(
    std::shared_ptr<RpcServer> server, std::shared_ptr<Session> session,
    std::shared_ptr<MethodInstance> method_instance, const RpcRequest &request,
...
namespace internal {
co20::ValueTask<bool>
AnyStreamWriter::Write(std::unique_ptr<google::protobuf::Any> res) {
  if (IsCancelled()) {
    co_return false;
  }
