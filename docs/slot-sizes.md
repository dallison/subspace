...
single message. This document covers the two ways a build or a publisher can
change the range a slot size may take: the `SUBSPACE_64BIT_SLOT_SIZE` build
option, which raises the ceiling above 2GB, and the `max_slot_size` publisher
option, which lowers it to a value you choose.

## 64-bit Slot Sizes

Slot sizes are 64-bit values everywhere inside Subspace, but the C and C++
client APIs expose them as 32-bit values by default. Building with
`SUBSPACE_64BIT_SLOT_SIZE` widens those APIs so that a channel can have slots
larger than 2GB.
...
## Limiting the Slot Size

A channel resizes itself whenever a publisher asks `GetMessageBuffer()` for
more room than the current slot holds, and it never shrinks. One oversized
message therefore permanently grows the channel for everybody. Set
`max_slot_size` to bound that.

```cpp
auto pub = client.CreatePublisher("frames", subspace::PublisherOptions()
                                                .SetSlotSize(64 * 1024)
                                                .SetNumSlots(16)
                                                .SetMaxSlotSize(1024 * 1024));
```

The limit does three things:

- The initial slot size must not exceed it. `CreatePublisher()` fails with
  `InvalidArgument` if it does.
- `GetMessageBuffer(n)` with `n` above the limit fails with `InvalidArgument`
  instead of resizing. The channel is left untouched, so the publisher can
  keep using its current slot after handling the error.
- Automatic growth stops at the limit. The growth multiplier normally
  overshoots the requested size, so a channel with 256-byte slots and a
  384-byte limit that is asked for 300 bytes resizes to 384 rather than the
  512 it would otherwise pick.

`max_slot_size` is channel-wide policy rather than a per-publisher preference,
so the server rejects a publisher whose limit disagrees with the one already
established for the channel, in the same way it rejects a disagreement over
`fixed_size` or `max_publishers`. A publisher that leaves the limit at 0 cannot
attach to a channel that has one.

The value is rounded up to the channel's 64-byte alignment, the same as the
slot size, so that a publisher asking for `slot_size == max_slot_size` is not
rejected by its own limit. `Publisher::MaxSlotSize()` reports the rounded
value, and 0 when the channel is uncapped.

This is a different tool from `fixed_size`, which pins the channel at its
initial slot size and fails every resize. Use `fixed_size` when the message
size is known and constant, and `max_slot_size` when messages vary but you
need a ceiling on the memory the channel can reach.

The option is available in every client: `SetMaxSlotSize()` in C++,
`max_slot_size` in the C `SubspacePublisherOptions`, `set_max_slot_size()` in
Python, and `set_max_slot_size()` in Rust. In the C and C++ APIs its type
follows `SlotSizeType`, so an uncapped default build cannot express a limit
above 2GB. Since a limit above 2GB is only meaningful on a channel whose slots
can exceed 2GB, that pairs naturally with building for 64-bit slot sizes.
