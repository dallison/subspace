...
#define EVAL_AND_ASSERT_OK(expr) \
  EVAL_AND_ASSERT_OK2(VAR(r_), expr)
...
TEST_F(StressTest, Coroutines) {
    ...
    struct Client {
        explicit Client(std::shared_ptr<subspace::Client> client) : client(std::move(client)) {}
        ...
        for (int j = 0; j < kNumChannels; j++) {
            clients[i].pubs.emplace_back(
                    EVAL_AND_ASSERT_OK(clients[i].client->CreatePublisher(
                            clients[i].channels[j], {.slot_size = 32, .num_slots = kNumSlots})));
        }
    ...
    auto publish_all = [&](co::Coroutine* c, int client_id) {
        for (int i = 0; i < kNumChannels; i++) {
            coroutines.push_back(std::make_unique<co::Coroutine>(
                    scheduler,
                    [&publish, client_id, i](co::Coroutine* c1) { publish(c1, client_id, i); },
    ...
    for (int i = 0; i < kNumClients; i++) {
        coroutines.push_back(std::make_unique<co::Coroutine>(
                scheduler,
                [&publish_all, i](co::Coroutine* c) { publish_all(c, i); },
    ...
    auto read = [&](co::Coroutine* c, int client_id, int subId) {
        ...
        while (num_messages < kNumSlots - 1) {
            ...
            for (;;) {
                absl::StatusOr<Message> m = clients[client_id].subs[subId].ReadMessage();
                ASSERT_OK(m);
                auto msg = *m;
                if (msg.length == 0) {
                    break;
    ...
    for (int i = 0; i < kNumClients; i++) {
        coroutines.push_back(std::make_unique<co::Coroutine>(
                scheduler, [&readAll, i](co::Coroutine* c) { readAll(c, i); }, "readall"));
    }
        ...
        for (int j = 0; j < kNumChannels; j++) {
            clients[i].subs.emplace_back(
                    EVAL_AND_ASSERT_OK(clients[i].client->CreateSubscriber(
                            clients[i].channels[j], {.max_active_messages = kMaxActiveMessages})));
        }
    }
...
TEST_F(StressTest, ActiveMessages) {
    ...
    client.sub = std::make_unique<subspace::Subscriber>(
            EVAL_AND_ASSERT_OK(client.client->CreateSubscriber(
                    client.channel, {.max_active_messages = kMaxActiveMessages, .log_dropped_messages = false})));
    ...
    for (int i = 0; i < kMaxActiveMessages; i++) {
        ...
        receivers[i].thread = std::thread([&, i]() {
            while (num_messages_received < kNumMessages - num_dropped_messages.load()) {
                auto msg = receivers[i].pipe.Read();
                num_messages_received++;
                // Just throw the message away.
                // We are interested in the receiver reading the message, not what the message
                // contains.
    ...
    auto publish = [&]() {
        for (int i = 0; i < kNumMessages; i++) {
            ...
            ASSERT_OK(pubStatus);
            // Sleep for a small amount of time to allow the receiver to read the message.
    ...
    auto read = [&]() {
        ...
        int32_t last_slot = -1;
        while (total_messages_received < kNumMessages - num_dropped_messages.load()) {
            // Read all available messages up to our active message limit.
            absl::StatusOr<std::vector<::subspace::Message>> msgs = client.sub->GetAllMessages();
            ASSERT_OK(msgs);
            auto all = *msgs;
            for (auto& msg : all) {
                if (msg.length == 0) {
    ...
    std::cerr << "subscribers received " << total_messages_received << " messages" << std::endl;
    std::cerr << "we dropped " << num_dropped_messages << " messages" << std::endl;
    // Send one more messages to wake up the receivers.
    for (auto& r : receivers) {
        subspace::Message msg;
        ASSERT_OK(r.pipe.Write(std::make_shared<subspace::Message>(std::move(msg))));
    }
    ...
    std::cerr << "receivers received " << (num_messages_received - receivers.size() + 1)
              << " messages" << std::endl;
...
TEST_F(StressTest, ManyChannelsNonMultiplexed) {
    ...
    for (int i = 0; i < kNumChannels; i++) {
        absl::StatusOr<Subscriber> sub =
                sub_client->CreateSubscriber(channels[i], {.log_dropped_messages = false});
    ...
    std::cerr << "Dropped " << (kNumMessages * kNumChannels) - num_messages << " messages\n";
}
...
TEST_F(StressTest, ManyChannelsMultiplexed) {
    ...
    for (int i = 0; i < kNumChannels; i++) {
        absl::StatusOr<Publisher> pub = pub_clients[i]->CreatePublisher(
                channels[i], {.slot_size = kSlotSize, .num_slots = knum_slots, .mux = kMux});
        ASSERT_TRUE(pub.ok());
        pubs.push_back(std::move(*pub));
    }
...
TEST_F(StressTest, ManyChannelsMultiplexedSubscribedToMux) {
    ...
    for (int i = 0; i < kNumChannels; i++) {
        absl::StatusOr<Publisher> pub = pub_clients[i]->CreatePublisher(
                channels[i],
                {.slot_size = kSlotSize, .num_slots = knum_slots, .activate = true, .mux = kMux});
        ASSERT_TRUE(pub.ok());
        pubs.push_back(std::move(*pub));
    }
