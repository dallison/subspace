...
absl::Status RpcClient::OpenBlocking(std::chrono::nanoseconds timeout) {
  ...
  boost::asio::co_spawn(
      ioc,
      [this, timeout, &result]() -> boost::asio::awaitable<void> {
...
RpcClient::PublishServerRequest(const subspace::RpcServerRequest &req,
  ...
  for (;;) {
    ...
    auto wait_status = co_await async_wait_readable_timeout(pub_fd, timeout);
    if (!wait_status.ok()) {
      co_return absl::InternalError(absl::StrFormat(
...
RpcClient::ReadServerResponse(int request_id,
  ...
  for (;;) {
    ...
    auto wait_status = co_await async_wait_readable_timeout(sub_fd, timeout);
    if (!wait_status.ok()) {
      co_return absl::InternalError("Timeout waiting for response");
    }
    for (;;) {
...
RpcClient::OpenService(std::chrono::nanoseconds timeout) {
  ...
  auto resp = co_await ReadServerResponse(request_id, timeout);
  if (!resp.ok()) {
    co_return absl::InternalError(absl::StrFormat(
...
RpcClient::InvokeMethod(int method_id, const google::protobuf::Any &request,
  ...
  *req.mutable_argument() = request;
  ...
  for (;;) {
    ...
    auto wait_status =
        co_await async_wait_readable_timeout(pub_fd, timeout);
    if (!wait_status.ok()) {
      co_return absl::InternalError(absl::StrFormat(
          "Failed to wait for message buffer: %s", wait_status.ToString()));
    }
  ...
  for (;;) {
    ...
    auto wait_status =
        co_await async_wait_readable_timeout(sub_fd, timeout);
    if (!wait_status.ok()) {
      co_return absl::InternalError(absl::StrFormat(
...
boost::asio::awaitable<absl::Status> RpcClient::InvokeMethod(
